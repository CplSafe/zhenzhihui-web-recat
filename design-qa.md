...
- 参考稿：`codex-clipboard-4d5a9a59-6884-4475-a494-106979c6b717.png`（1919 × 1080）
- 已核对：品牌区、点阵背景、五项浮动工具栏、素材/历史入口、节点标题、节点卡片、连接加号、底部生成面板。
- 功能检查：画布类型检查、相关单元测试、涉及文件 ESLint 均通过。
- 待人工视觉复核：当前环境没有可用的应用内浏览器截图能力，无法对运行页面执行像素级叠图比较；需要在 1919 × 1080 视口打开 `/canvas/:id` 后复核实际字体渲染、节点初始位置和缩放比例。
