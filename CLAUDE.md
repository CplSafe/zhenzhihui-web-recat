...
- **`src/components/`** — grouped by domain: `auth`, `billing`, `layout`, `home` (`AppSidebar`), `creative` (storyboard/timeline/video panels for the legacy flow), `smart` (the 智能成片 2.1 step UIs — each in its own subfolder), `common` (small shared widgets: `AiBadge`, `InlineEdit`, `EllipsisText`), `material`, `resource`, `space`, `team`. Global singletons `AppToast` and `AppConfirmDialog` are mounted once in `AppShell`.
