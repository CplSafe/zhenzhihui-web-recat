...
interface HotCopyEntryProps {
  ...
  /** 已有生成结果时为 true:发送区显示「重新生成」+「下一步」(从上一步退回入口的场景) */
  canResume?: boolean
  /** 「下一步」:回到已生成的视频步,不重新生成 */
  onResume?: () => void
}
...
export default function HotCopyEntry({ onSubmit, initial, canResume, onResume }: HotCopyEntryProps) {
      ...
      <div className="hotcopy__panel">
        ...
        <div className="hotcopy__card">
          ...
          <div className="hotcopy__bottom">
            ...
            {canResume ? (
              // 恢复态(从「上一步」退回入口、已有生成结果):原发送按钮 → 重新生成,并多一个「下一步」回到已生成视频
              <div className="hotcopy__resume-actions">
                <button
                  type="button"
                  className="hotcopy__resume-btn hotcopy__resume-btn--ghost"
                  onClick={submit}
                  title="按当前的源视频 + 替换素材重新生成"
                >
                  重新生成
                </button>
                <button
                  type="button"
                  className="hotcopy__resume-btn hotcopy__resume-btn--primary"
                  onClick={() => onResume?.()}
                  title="下一步:回到已生成的视频(不重新生成)"
                >
                  下一步
                </button>
              </div>
