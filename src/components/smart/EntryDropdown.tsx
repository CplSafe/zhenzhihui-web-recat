...
export default function EntryDropdown({
  ...
  return (
    <div className="entrydd" ref={ref}>
      ...
      >
        {icon}
        <span className="entrydd__val">{label}</span>
        <svg className="entrydd__caret" viewBox="0 0 10 6" width="10" height="6" aria-hidden="true">
          <path d="M1 1l4 4 4-4" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
        </svg>
      </button>
      {open && (
        <div className="entrydd__menu" role="listbox" aria-multiselectable={multiple}>
          {options.map((o) => (
            <button
              key={o}
              type="button"
              ...
              className={`entrydd__opt${isSel(o) ? ' is-active' : ''}${multiple ? ' entrydd__opt--multi' : ''}`}
              onClick={() => handlePick(o)}
            >
              {multiple && (
                <span className="entrydd__check" aria-hidden="true">
                  {isSel(o) && (
