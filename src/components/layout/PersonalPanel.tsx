...
const roleValueOf = (member: any, workspace: any, user: any): string => {
  ...
  const memberWorkspaceId = Number(
    member?.workspace_id ?? member?.workspaceId ?? member?.workspace?.id ?? member?.current_workspace_id ?? member?.currentWorkspaceId ?? 0,
  )
  ...
  return String(member?.workspace_role || member?.workspaceRole || member?.member_role || member?.memberRole || member?.role || '')
    .trim()
    .toLowerCase()
}
      ...
      body: JSON.stringify({
        ...
        data: {
          ...
          currentWorkspace: currentWs
            ? { id: currentWs.id, name: currentWs.name, type: currentWs.type }
            : null,
