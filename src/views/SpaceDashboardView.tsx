...
export default function SpaceDashboardView() {
  ...
  const currentRole = String(
    currentMember?.workspace_role || currentMember?.workspaceRole || currentMember?.role || currentMember?.member_role || '',
  )
    .trim()
    .toLowerCase()
  const canRevealWorkspaceInfo = isPersonalWorkspace || currentRole === 'member' || currentRole === 'admin' || (ownerUserId > 0 && currentUserId === ownerUserId)
  const canViewDashboard = !isPersonalWorkspace && (currentRole === 'admin' || (ownerUserId > 0 && currentUserId === ownerUserId))
  ...
  return (
    <div className="space-dashboard-page">
      <AppSidebar activeKey="team" onNavigate={handleNavigate} open={sidebarOpen} onClose={() => setSidebarOpen(false)} />
