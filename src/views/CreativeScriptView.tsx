...
function CreativeScriptViewBody(props: CreativeScriptViewProps): ReactNode {
  ...
  const [generationPending, setGenerationPending, generationPendingRef] = useStateRef(false)
  ...
  const [isGenerating, setIsGenerating, isGeneratingRef] = useStateRef(false)
  const [isSavingDraft, setIsSavingDraft, isSavingDraftRef] = useStateRef(false)
  ...
  const materialStore = useMaterialLibraryStore
  ...
  const [timelineAutoGenerated, setTimelineAutoGenerated, timelineAutoGeneratedRef] = useStateRef(false)
  ...
  /**
   * 链式逐张生成分镜脚本词。
   * 每张分镜生成时都会带上前面已生成分镜的完整内容作为上下文，
   * 确保叙事连贯、主体/场景/风格一致，避免重复画面。
   */
  async function generateChainScript() {
    if (isSubmittingScriptRef.current) return
    ...
    setIsSubmittingScript(true)
    setIsScriptStreaming(true)
    showToastRef.current('创意脚本逐张生成中', 'success')

    const baseIdea = generatedPromptRef.current
    const totalSec = Number.parseInt(String(selectedDurationRef.current), 10) || 10
    const targetCount = Math.min(Math.max(Math.ceil(totalSec / 2), 3), MAX_STORYBOARDS)
    ...
    try {
      ...
      for (let i = 0; i < targetCount; i++) {
        const prompt = buildChainBoardPrompt({
          boardIndex: i,
          totalBoards: targetCount,
          allPreviousBoards: generatedBoards,
          baseIdea,
        })
        ...
        for (let attempt = 0; attempt < 2 && !parsed; attempt++) {
          try {
            ...
            parsed = parseStoryboardFromAiText(text)
            ...
            if (!parsed && attempt === 0) {
              ...
              const retryPrompt = [
                '你是一名短视频分镜导演。',
                `项目需求：${baseIdea}`,
                `这是第${i + 1}张分镜（共${targetCount}张），请直接输出分镜画面描述和脚本词。`,
                generatedBoards.length > 0
                  ? `前面已生成：${generatedBoards.map((b, j) => `分镜${j + 1}：${b.prompt}`).join('；')}`
                  : '',
                '输出 JSON：<<<STORYBOARD_JSON>>>[{"title":"标题","prompt":"画面描述","duration":2,"voiceover":"旁白","subtitle":"字幕","sfx":"音效"}]<<<END_STORYBOARD_JSON>>>',
              ]
                .filter(Boolean)
                .join('\n')
              try {
                const retryResult = await requestCreativeScriptWithFallback({ workspaceId: id, prompt: retryPrompt })
                parsed = parseStoryboardFromAiText(retryResult?.text)
              } catch {
                // 降级到兜底
              }
            }
        ...
        generatedBoards.push(parsed)
        setCreativeStoryboards([...generatedBoards])
      }

      // 最终格式化 generatedScript 用于展示
      setGeneratedScript(
        generatedBoards
          .map(
            (b, j) =>
      ...
      if (generatedBoards.length > 0) {
        // 部分生成成功
        setCreativeStoryboards([...generatedBoards])
        setGeneratedScript(generatedBoards.map((b, j) => `【分镜${j + 1}】${b.title}\n画面：${b.prompt}\n`).join('\n'))
        showToastRef.current(`创意脚本部分生成（已完成 ${generatedBoards.length}/${targetCount} 张）`, 'error')
      } else {
        setIsGenerating(false)
        setGeneratedScript('')
        setCreativeStoryboards([])
