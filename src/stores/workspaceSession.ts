...
const sanitizeWorkspaceList = (list: any[]): any[] => dedupeWorkspaces((Array.isArray(list) ? list : []).filter(isVisibleWorkspace))
const deriveSessionWorkspaces = (session: any): any[] =>
  sanitizeWorkspaceList(dedupeWorkspaces(session?.workspaces, session?.workspace, session?.currentWorkspace, session?.current_workspace))
  ...
  return {
    ...
    loadSubscriptionLabel: async () => {
      ...
      const [sub, wal] = await Promise.all([
        ...
        get().loadCurrentWorkspaceMember(id).catch(() => null),
      ])
