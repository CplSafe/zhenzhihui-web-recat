/*
  useCreativeWorkflow — 创意脚本全流程编排 headless hook
  从 CreativeScriptView 抽出全部编排逻辑（state / ref / 子 hook 调用 / 函数 / useEffect）。
  视图（CreativeScriptView）只负责调用本 hook 并渲染。
  注意：声明顺序对 TDZ / 函数提升敏感，整段按原顺序保持不变。
*/
...
interface UseCreativeWorkflowProps {
  // 原视图通过 props auth-session / emit('logout-success') 与 App 通信，
  // React 改用 useAuth()。logout 仍可由父级透传回调。
  onLogoutSuccess?: () => void
}

export function useCreativeWorkflow(props: UseCreativeWorkflowProps) {
