...
/** 手持工具、拿取物品等精细接触镜头的通用分镜约束。 */
const PHYSICAL_INTERACTION_STORYBOARD_RULES =
  '【高风险物理交互】凡涉及手持工具、拿取或传递物品、穿戴配件、体育器械接触、机械部件操作、开合拆装或多人肢体接触的镜头,' +
  '每个镜头只安排一个主要接触动作;时长分配允许时优先控制在2至4秒,复杂过程必须拆成“建立正确接触”和“执行单一动作”等相邻镜头。' +
  '这类镜头的desc必须依次写清:操作者、标准工具名称与可见关键结构、直接受力对象、精确接触部位、初始接触和遮挡关系、单一动作、结束接触状态、景别机位。' +
  '凡出现旋转、拧紧、拧松、顺时针或逆时针,desc必须同时说明:被旋转或固定的对象、旋转中心轴、运动平面、判断顺逆时针的镜头观察方向、圆弧角度以及工具和手部是否同步运动;不得只写“顺时针拧动”。' +
  '工具作用对象必须使用准确名词:活动扳手夹持六角螺母或六角螺栓头的相对两个平面,螺丝刀作用于匹配的螺钉槽口;信息不足时使用“六角紧固件”,不得混写螺丝、螺母和螺栓。' +
  '活动扳手必须描述为一个固定钳口、一个活动钳口、一个调节蜗轮和一根完整手柄,两个钳口位于同一平面并形成唯一开口;执行旋转的镜头中开口宽度保持不变。' +
  '例如不要写“手持扳手顺时针拧螺丝”,应写“从镜头俯视方向观察,活动扳手两个钳口夹住六角螺母相对的两个平面,螺母轴线垂直工作台,手与扳手绕该轴在平行工作台的水平面内同步顺时针转动约30度,不得上下提拉”。' +
  '接触操作优先使用固定特写、固定近景或轻微跟随,不安排环绕、快速摇镜或大幅运镜。' +
  '允许为了正确接触而微调手指、手腕、工具角度和目标物的局部位置,不得同时要求它们绝对不动。' +
  '正向描述正确结构、接触与受力关系为主,末尾简洁限制结构缺失、多余部件、穿模、悬浮、错位、形变、闪烁或物体替换。' +
  '相邻镜头必须沿用同一人物、同一物体、同一服装、同一场景及上一镜头的结束接触状态。' +
  '不得在同一个短镜头中同时安排“拿起工具、调节开口、建立夹持、旋转、松开、展示结果”等多个阶段;应拆为建立接触和执行动作等相邻镜头。' +
  '如镜头不涉及物理接触,不要强行添加工具结构或接触描述。'

/** 常见动作场景必须补齐的可执行视觉语义，避免只写抽象结果。 */
const SCENE_ACTION_STORYBOARD_RULES =
  '【场景动作语义】根据镜头实际内容按需应用,不得把无关类别塞进分镜:' +
  '手与物体操作写清接触部位、抓握方式、移动轨迹和放置终点;' +
  '人物走跑跳坐转身写清朝向、支撑脚、重心变化、路径和结束姿态;' +
  '体育动作写清器械接触、发力链、释放时刻、飞行或滚动轨迹和目标位置;' +
  '烹饪操作写清工具刃口或容器开口、食材接触面、动作方向和操作结果;' +
  '产品开合、挤压、涂抹、佩戴或组装写清可动部件、作用位置、前后状态和产品外观连续性;' +
  '多人互动写清双方身体部位、先后顺序、接触点、遮挡和分离后的状态;' +
  '穿戴配饰写清左右侧、佩戴部位、开合结构、穿戴路径和最终朝向;' +
  '驾驶骑行写清人物与座椅、方向盘、车把或踏板的接触,车辆行驶方向与环境视差;' +
  '液体、粉末、烟雾或火焰写清来源、出口、重力或气流方向、连续轨迹、落点和前后体积变化;' +
  '镜头推拉摇移跟拍环绕写清相机运动方向、速度、主体在画面中的相对位置和稳定方式。' +
  '描述必须呈现可观察的动作过程,不要只写“完成操作”“自然运动”“展示效果”等抽象结果。'
...
const SYSTEM =
  ...
  // 视频模型画不了中文——desc 里设计「屏幕大字/促销字样」只会在成片里渲染成乱码
  '画面描述(desc)只写可拍摄的画面内容,不得设计出现在画面中的文字元素(大字标题、促销字样、标语、价格牌特写等);文字信息一律放进字幕(subtitle)字段。' +
  '若提供了素材图片,务必逐张判断它对应哪个主体:对应的主体必须加 imageIndex 字段(从1开始,表示第几张素材图),' +
  '尽量让每张素材图都被某个主体引用(场景图配场景主体、产品图配产品主体);确实无对应的主体才省略该字段。' +
  PHYSICAL_INTERACTION_STORYBOARD_RULES +
  SCENE_ACTION_STORYBOARD_RULES +
